...
| `c_conserved_order_parameter` | implemented_as_conserved_order_spectral_v1_and_bridge_passed | Core spectral bridge exists, but the latest controller is structure-factor domain-scale calibration. | Run multi-grid structure-factor calibration/replication before exponent or universality claims. |
...
`Run multi-grid structure-factor calibration for 0.11 before adding new warped-space or dynamic-game operators.`

Wave 24 narrowed the active evidence chain to estimator/domain-scale calibration; For Work prefers narrowing the current controller before broadening scope.
