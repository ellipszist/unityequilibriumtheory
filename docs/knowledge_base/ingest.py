...
======================
Scan → Tensorize → Store — bulk pipeline for docs/topics/
...
Usage:
    # Dry-run:
    python -m docs.knowledge_base.ingest --dry-run

    # Full ingestion:
    python -m docs.knowledge_base.ingest

    # Single topic:
    python -m docs.knowledge_base.ingest --topic 0.0_Grand_Unification
...
class IngestionPipeline:
    ...
    Scans docs/topics/ → tensorizes each file → stores in VectorStore.
    """
...
def main():
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        default=str(project_root / "docs" / "knowledge_base" / "vectors"),
