...
| `Data/03_Research/atomic_predictive_v1_operator_implementation_record_manifest.json` | Predictive-v1 review-only implementation record for selected operator class and code identity | SHA256, selected operator class, target module path, runtime source hash, required entrypoint, required return key, matching review-only parameter-set count, and implementation-record-only claim boundary recorded in artifact |
...
- Atomic predictive-v1 operator-implementation-record check count, pass count, blocking count, matching review-only parameter-set count, and implementation-record status.
...
- atomic predictive-v1 operator-implementation-record manifest, selected operator class, target module path, runtime source hash, required entrypoint, required return key, matching review-only parameter-set count, explicit implementation-record-ready status, and implementation-record-only claim boundary
