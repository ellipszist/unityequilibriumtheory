...
- The predictive-v1 operator-class selection review gate sits one step before candidate promotion resolves `selected_operator_class`. It may recommend the fixed CI/correlated lane first because that lane is already priority-1 in the build spec and does not wait on a new UET derivation artifact, but it does not itself select the class or create an implementation claim.
