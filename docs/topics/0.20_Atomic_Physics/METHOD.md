...
| Precision spectroscopy targets | `hydrogen_precision_spectroscopy_sources.json`, `hydrogen_lamb_shift_correction_sources.json`, `hydrogen_hyperfine_21cm_sources.json`, precision artifacts | source package plus nonrelativistic, leading Dirac, empirical Lamb-handoff, and 21 cm bookkeeping diagnostics; QED/recoil/proton-size/hyperfine model blocked |
| Neutral helium / many-electron targets | `helium_many_electron_sources.json`, artifact `helium_many_electron_gate` | source package only; model blocked |
...
12. Load 21 cm hyperfine source rows and compute frequency-to-wavelength bookkeeping, not the hyperfine Hamiltonian.
13. Load neutral helium source rows and many-electron model requirements as source-package rows only.
14. Write artifact with hashes, thresholds, metrics, formula bridge metadata, level-energy rows, selected ion rows, precision source/baseline gates, Dirac baseline gate, Lamb handoff gate, 21 cm source gate, neutral helium source gate, and limitations.
...
- Precision spectroscopy rows support a source package plus nonrelativistic, leading Dirac, empirical Lamb-handoff 1S-2S residuals, and 21 cm source bookkeeping only until QED/recoil/proton-radius/hyperfine models and uncertainty propagation are added.
...
- Precision source-package readiness language for 1S-2S, Lamb shift, and 21 cm hyperfine targets, plus nonrelativistic, leading Dirac, empirical Lamb-handoff, and 21 cm source-bookkeeping diagnostic language.
