...
| Predictive v1 operator implementation provenance | `atomic_predictive_v1_operator_implementation_provenance.json`, artifact `atomic_predictive_v1_operator_implementation_provenance_gate` | narrows the accepted-operator blocker into 5 required provenance artifacts: code identity, parameter lock, training/holdout split, accepted residual emitter, and operator uncertainty provenance |
...
- The predictive-v1 operator implementation-provenance gate records `5` required provenance evidence rows and all `5` remain blocking. It requires accepted operator code identity, non-empty locked parameter provenance, training/holdout split, accepted residual emitter, and operator uncertainty provenance before any diagnostic rows can be reclassified.
