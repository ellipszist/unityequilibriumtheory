...
14. Load neutral helium source rows and many-electron model requirements as source-package rows only.
15. Write artifact with hashes, thresholds, metrics, formula bridge metadata, level-energy rows, selected ion rows, precision source/baseline gates, Dirac baseline gate, Lamb handoff gate, 21 cm source/Fermi gates, neutral helium source gate, and limitations.
