...
| Predictive v1 operator implementation provenance | `atomic_predictive_v1_operator_implementation_provenance.json`, artifact `atomic_predictive_v1_operator_implementation_provenance_gate` | narrows the accepted-operator blocker into 5 required provenance artifacts; the training/holdout split is now present, while code identity, non-empty locked parameters, accepted residual emitter, and operator uncertainty provenance remain missing |
...
- The predictive-v1 operator implementation-provenance gate records `5` required provenance evidence rows. `1/5` is now present through the split manifest; `4/5` remain blocking: accepted code identity, non-empty locked parameters, accepted residual emitter, and operator uncertainty provenance.
