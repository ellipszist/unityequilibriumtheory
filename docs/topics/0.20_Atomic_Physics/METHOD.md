...
- Neutral helium rows support source targets, photon-energy bookkeeping, term assignments, medium normalization, source component policy, ground-state baseline residual diagnostics, excited-state target preparation, zero-quantum-defect residual sizing, and limited source-calibrated quantum-defect prediction diagnostics only until a correlated two-electron Hamiltonian/spectral model, independent holdout set, uncertainty propagation, resolved line-shape policy for precision use, and residual thresholds are added.
...
- Neutral helium source-package readiness, photon-energy, term-assignment, medium-normalization, component-policy, ground-state baseline, excited-state target, hydrogenic residual, and quantum-defect prediction blocker language for future many-electron artifacts.
