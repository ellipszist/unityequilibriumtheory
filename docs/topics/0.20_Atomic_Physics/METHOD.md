...
- The predictive-v1 operator parameter preflight gate records `5` field-level acceptance checks for future operator parameters. It currently has `0` parameter sets and `5/5` blocking preflight checks, so PROV-02 remains blocked by an empty accepted parameter set.
