...
- The predictive-v1 operator parameter preflight gate records `5` field-level acceptance checks for future operator parameters. It currently has `0` parameter sets, so `1/5` checks block directly on the missing accepted parameter set while the other `4/5` remain not yet evaluable; a first candidate parameter-set blueprint now names the required rows, source hashes, and forbidden sources before PROV-02 can move.
