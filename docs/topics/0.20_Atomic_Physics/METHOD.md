...
| Predictive v1 operator implementation provenance | `atomic_predictive_v1_operator_implementation_provenance.json`, artifact `atomic_predictive_v1_operator_implementation_provenance_gate` | narrows the accepted-operator blocker into 5 required provenance artifacts; the training/holdout split is present, and the remaining 4 blockers now carry candidate evidence paths/hashes plus blocker reasons |
...
35. Build a predictive-v1 operator implementation-provenance gate that records the evidence required before residual rows can count as accepted `delta_uet_or_ci` output.
36. Build a predictive-v1 diagnostic report gate that collects the selected-lane predictions, parameter-lock state, threshold rows, operator-contract state, lineage decision, and validation blockers.
37. Build a predictive-model blueprint gate that turns the build path into auditable steps: domain lane, model equation, parameter lock, holdout protocol, baseline comparator, uncertainty threshold, and source lineage.
...
- The predictive-v1 operator parameter preflight gate records `5` field-level acceptance checks for future operator parameters. It currently has `0` accepted parameter sets, so `1/5` checks block directly on the missing accepted parameter set while the other `4/5` remain not yet evaluable; the manifest now also carries `1` candidate parameter set with calibration rows, source hashes, forbidden sources, and placeholder uncertainty state, but it remains explicitly unaccepted.
- The predictive-v1 parameter candidate promotion gate sits one step before accepted `parameter_sets`. It reviews whether the current candidate has a selected operator class, non-placeholder parameter rows, a real lock timestamp, and source bindings consistent with the frozen split before promotion is even considered.
