...
| Fixed-parameter model readiness | Lane-wise model-readiness matrix | `atomic_fixed_parameter_model_readiness_gate` in artifact | Eight lanes separate fixed standard baselines, empirical handoffs, fitted diagnostics, and missing required generative models. | Two required model lanes are missing: CI/correlated helium and explicit UET atomic operator. |
