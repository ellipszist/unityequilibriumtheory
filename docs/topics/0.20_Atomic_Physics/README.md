...
| Neutral helium / many-electron | Source package plus partial term assignments | `Data/03_Research/helium_many_electron_sources.json`, `helium_transition_assignments.json`, `helium_transition_assignment_gap_gate` in artifact | Five He I visible target lines have photon energies computed (`~1.755-3.188 eV`); 4/5 rows now have NIST term assignments. | The 447.148 nm row remains unassigned; no two-electron Hamiltonian/correlation residual gate exists yet. |
...
It must also record `atomic_claim_scope_gate`, which allows only hydrogen Rydberg benchmark, rounded hydrogen level-energy benchmark, formula-bridge-manifest, provisional selected He+/Li2+ reduced-mass benchmark language, C VI stress-test language, precision-source-package language, nonrelativistic/Dirac 1S-2S baseline diagnostic language, empirical Lamb-handoff language, 21 cm source-bookkeeping/Fermi-baseline language, and neutral-helium source/transition-blocker language while blocking Rydberg derivation, broad hydrogen-like ion validation, QED correction validation, neutral helium residual validation, many-electron, and full atomic-theory claims.
...
- `Data/03_Research/helium_transition_assignments.json`: partial NIST persistent-line term assignments for He I rows.
