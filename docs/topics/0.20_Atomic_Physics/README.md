...
| Atomic residual uncertainty budget | Source-uncertainty residual-budget rows where current source uncertainties permit computation | `atomic_residual_uncertainty_budget_gate` in artifact | Eight budget rows make residual-to-source-uncertainty ratios and missing source-uncertainty rows explicit. | Six computable rows still exceed source uncertainty; two helium holdout rows lack source uncertainty, so no uncertainty-qualified prediction claim is allowed. |
