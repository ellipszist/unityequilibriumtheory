...
| Helium external holdout acquisition | Source-acquisition gate for the first candidate lane | `helium_external_holdout_acquisition_gate` in artifact | Identifies CHIANTI He I as an external database cross-check candidate with two raw files, two SHA256 hashes, and two overlapping lines (`522.21309 A`, `537.02992 A`). | Not accepted as independent validation until source-lineage review and uncertainty-aware thresholds are complete. |
| Helium external holdout residual cross-check | Cross-source delta gate for the first candidate lane | `helium_external_holdout_residual_crosscheck_gate` in artifact | Computes 2 CHIANTI-vs-current holdout deltas; max wavelength delta is `~0.172 ppm`, max upper-energy delta is `~0.104 cm^-1`. | Diagnostic only; CHIANTI lineage and acceptance thresholds remain blocked, so this does not validate helium prediction. |
