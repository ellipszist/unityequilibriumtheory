...
| Neutral helium / many-electron | Source package plus term assignments and medium normalization | `Data/03_Research/helium_many_electron_sources.json`, `helium_transition_assignments.json`, `helium_transition_assignment_gap_gate`, `helium_medium_normalization_gate` in artifact | Five He I visible target lines have photon energies computed (`~1.755-3.188 eV`); 5/5 rows now have NIST term assignments and air/vacuum normalization. | No two-electron Hamiltonian/correlation residual gate exists yet. |
...
- `Data/03_Research/helium_transition_assignments.json`: NIST handbook/ASD term assignments for selected He I rows.
