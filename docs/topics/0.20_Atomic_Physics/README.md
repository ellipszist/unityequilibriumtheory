...
| Precision spectroscopy | Source package only | `Data/03_Research/hydrogen_precision_spectroscopy_sources.json`, `precision_spectroscopy_gate` in artifact | 1S-2S, Lamb shift, and 21 cm hyperfine targets are organized for future artifacts. | No Dirac/QED/recoil/proton-radius/hyperfine correction model or residual gate yet. |
...
It must also record `atomic_claim_scope_gate`, which allows only hydrogen Rydberg benchmark, rounded hydrogen level-energy benchmark, formula-bridge-manifest, provisional selected He+/Li2+ reduced-mass benchmark language, and precision-source-package language while blocking Rydberg derivation, broad hydrogen-like ion validation, QED correction validation, neutral helium, many-electron, and full atomic-theory claims.
