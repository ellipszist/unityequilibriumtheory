...
| Helium external holdout acquisition | Source-acquisition gate for the first candidate lane | `helium_external_holdout_acquisition_gate` in artifact | Identifies CHIANTI He I as an external database cross-check candidate with two overlapping lines (`522.21309 A`, `537.02992 A`). | Not accepted as independent validation until raw files/hashes, line locators, uncertainty policy, and source-lineage review are complete. |
