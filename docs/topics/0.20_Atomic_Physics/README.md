...
| Neutral helium / many-electron | Source package plus transition-assignment blocker | `Data/03_Research/helium_many_electron_sources.json`, `helium_many_electron_gate`, `helium_transition_assignment_gap_gate` in artifact | Five He I visible target lines have photon energies computed (`~1.755-3.188 eV`). | All rows still lack upper/lower term assignments, so no neutral-helium residual gate or two-electron Hamiltonian validation exists yet. |
