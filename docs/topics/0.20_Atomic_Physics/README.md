...
| Precision spectroscopy | Source package plus baseline diagnostics | `Data/03_Research/hydrogen_precision_spectroscopy_sources.json`, `hydrogen_lamb_shift_correction_sources.json`, `hydrogen_hyperfine_21cm_sources.json`, and precision gates in artifact | 1S-2S, Lamb shift, and 21 cm hyperfine targets are organized; nonrelativistic, leading Dirac, empirical Lamb-handoff, and 21 cm bookkeeping gates are computed. | 1S-2S residuals step from `~22.99 GHz` to `~7.10 GHz` to `~10.32 MHz`; 21 cm wavelength is `~21.106114054 cm`; these are not QED/recoil/proton-size/hyperfine Hamiltonian validation. |
