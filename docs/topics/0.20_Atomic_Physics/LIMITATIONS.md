...
- Neutral helium rows are source-package targets with photon energies, NIST term assignments, and wavelength-medium normalization computed only. The two-electron Hamiltonian, correlation treatment, selection-rule policy, uncertainty propagation, and many-electron residual model are not primary-gated yet.
