...
- Neutral helium rows are source-package targets with photon energies, NIST term assignments, wavelength-medium normalization, line-component/blend policy, ground-state baseline residual diagnostics, an external correlated ground-state reference target, excited-state target energies, and zero-quantum-defect hydrogenic residuals computed only. The independent-electron baseline intentionally overbinds, the variational effective-charge baseline remains uncorrelated, and the zero-quantum-defect excited baseline exposes missing singlet/triplet/quantum-defect/correlation structure; no in-repo correlated two-electron spectral model, uncertainty propagation, resolved line-shape policy for precision use, or many-electron residual model is primary-gated yet.
