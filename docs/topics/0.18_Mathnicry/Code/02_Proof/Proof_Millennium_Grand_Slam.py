...
def run_grand_slam():
    ...
    if p1 and p2 and p3:
        ...
        logger.log_step(
            step=1,
            time_val=0.0,
            omega=0.0,  # Success state
            kinetic=0.0,
            entropy=0.0,
            gradient=0.0,
            status="GRAND_SLAM_VERIFIED",
            details="All 7 Millennium Problems Unified under UET",
        )

        report_path = logger.save_report()
        print(f"✅ Glass Box Report saved to {report_path}")

        # Legacy Text Report for easy reading (Optional, but good for humans)
        result_dir = Path(report_path)
        ...
        print(f"📄 Human-Readable Report saved to {result_dir / 'grand_slam_report.txt'}")
