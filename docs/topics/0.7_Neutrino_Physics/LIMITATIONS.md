...
- The official NuFIT 6.0 benchmark is now tied to a local upstream artifact, but the extracted JSON is still a manual transcription layer and should be rechecked if parser automation is added later.
- The current pass does not mean the neutrino mass splittings are derived from first principles; they are still benchmark-fed runtime parameters.
