...
| `higgs_coupling_cms_2022_reference_package.json` | `docs/data/external/particle_physics/higgs/higgs_coupling_cms_2022_reference_package.json` | external package | extracted/source-referenced package from topic-local working benchmark | Extracted/source-referenced package; raw CMS table still not archived. | `mass_GeV` in GeV; `coupling_kappa_observed` and `uncertainty` dimensionless. | External provenance package for the primary verifier input. |
