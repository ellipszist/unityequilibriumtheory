...
- SEMF coefficients and Yukawa constants now have a local engine-extracted package with `0` gate mismatches, and the SEMF coefficient set has an exact external source-candidate match, but the direct source record is still not held locally, so decomposition does not make the topic parameter-free.
- `semf_coefficient_provenance_gate.json`, `semf_coefficient_local_package.json`, `semf_coefficient_source_candidates.json`, `semf_coefficient_provenance_diagnostic.json`, and `semf_coefficient_source_candidate_audit.json` make that coefficient/source-policy blocker machine-readable and explicitly block first-principles nuclear-binding wording.
