...
an ultimate financing instrument or bank settlement. The SEC recipient concordance then matches only exact unique recipient names to the SEC issuer registry: 17 name matches map to 15 CIKs with 235 annual accounting fact rows; 1,268 of 1,285 unique names remain explicit no-match. These profit, operating-cash-flow, capex, debt, dividend, and cash fields are firm accounting channels, not earmarked sources for an award dollar.
...
| Downstream subaward recipients | All ten fixed awards have complete count/page coverage; one award reports `3,168` subawards, yielding `3,168` rows and `1,024` unique downstream recipients; FY2024-derived slice has `950` rows | `0_25_usaspending_subaward_downstream_recipient_audit.json` | `PASS_WITH_BOUNDARY`; reported downstream-award visibility only, not settlement or invoice evidence |
...
| Recipient public-firm funding channels | 1,285 combined prime/subaward recipient names; 17 exact unique name matches; 15 SEC CIKs; 235 annual fact rows; no-match and no-imputation coverage preserved | `0_25_sec_recipient_funding_concordance_audit.json` | `PASS_WITH_BOUNDARY`; accounting channels only, funding share `NOT_IDENTIFIED` |
