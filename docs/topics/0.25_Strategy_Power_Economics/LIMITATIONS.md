...
- The payer-resource join gate is `BLOCKED`: the frozen package now validates a BEA 1997 benchmark make/use and requirements structure, a BLS public-API industry-hours sample, and selected USGS material quantities. Only 11 of 202 predeclared NAICS4 candidates returned BLS rows for 1987-2006 in the frozen archive; later API windows are unavailable because of the provider daily quota. USGS quantities are national and not industry-mapped, and SEC public-firm ratios cover only 10 firms from current-vintage 10-K facts. A project payment ledger remains unavailable. Aggregate energy throughput is present but is not a complete physical-resource ledger.
