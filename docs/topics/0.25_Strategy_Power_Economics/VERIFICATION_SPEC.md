...
- `0_25_usaspending_subaward_downstream_recipient_audit.json`: all-ten-award count hashes, every positive-count subaward page hash, reported-count versus retrieved-row reconciliation, normalized downstream recipient/amount/date/description fields, derived FY2024 slice, and explicit award-reporting/no-settlement boundary.
