...
- Research architecture: WARN; current package is Package Tier A, target is Evidence Grade A,
  ...
  quarantined.
- Payer-resource additions: USGS, SEC, the fixed USAspending DOE FY2024 public-award sample, and the Treasury aggregate funding-source snapshot are `PASS_WITH_BOUNDARY`; BLS industry-hours remains `WARN` for candidate-set coverage but its 11 returned series are complete for 1987-2024 (418 rows). The join gate reports bounded public inputs, while the private project payment ledger is still `BLOCKED`.
