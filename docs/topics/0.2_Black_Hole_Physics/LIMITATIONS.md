...
- The EHT shadow benchmark uses a compact GR-style shadow factor, not a full ray-traced image model.
- The internal saturation-core diagnostic uses a rescaled core radius for numerical visualization and cannot by itself establish physical singularity resolution.
...
- The artifact-level `black_hole_claim_scope_gate` is the export controller: EHT size
  benchmark PASS cannot be cited as singularity resolution, GR replacement, image-domain
  EHT validation, GW/ringdown validation, CCBH cosmological coupling, or black-hole
  information closure.
