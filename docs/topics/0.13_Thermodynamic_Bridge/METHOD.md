...
The Berut transcription-policy blocker narrows the same lane one step further: before claiming row-level closure, the repo should choose one explicit normalization path rather than implying that any future row extraction method would be equally trustworthy. The current decision note chooses `figure_level_locator_capture`, the figure-locator mapping note attaches `Figure 3` to the topic-summary row, the landmark-candidate pass records `jpeg_2` panel-frame candidates, and the semantic asset review now demotes `jpeg_3` to schematic/procedure support while selecting `jpeg_2` as the quantitative candidate. The next Berut controller is selected-panel tick/point mapping, or one stronger upstream numeric surface, rather than figure-locator or asset-role choice itself.
