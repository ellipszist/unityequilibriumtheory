...
2. Extend the current uncertainty artifact into a fuller package: Jun needs a source-backed uncertainty row, Berut needs raw-row provenance, and gravity-adjacent outputs still need direct 2022 measured-constant extraction plus systematic terms beyond the current mass-plus-`G`-proxy layer.
...
The Jun gap note narrows one step further by isolating the exact uncertainty fields still missing from the `Jun 2014` row.
The Jun mapping-conflict note narrows a different Jun problem: the current runtime `0.028 eV` row is not yet explicitly tied to one source-facing quantity in the `Jun 2014` paper.
The Hong-lineage note narrows a different problem again: the same legacy `0.028 eV / 44% above limit` row may belong partly or wholly to a later nanomagnetic-memory branch, so it cannot currently be treated as a clean `Jun 2014` row.
