...
BERUT_AUTHOR_HEAT_IMAGE_SURFACE_PATH = TOPIC_DIR / "Data" / "03_Research" / "berut_2012_author_heat_image_surface.json"
BERUT_AUTHOR_HEAT_IMAGE_RECORD_PATH = ROOT / "docs" / "data" / "external" / "thermodynamics" / "landauer" / "berut_2012" / "author_page_dissipated_heat_record.json"
BERUT_AUTHOR_HEAT_IMAGE_PATH = ROOT / "docs" / "data" / "external" / "thermodynamics" / "landauer" / "berut_2012" / "author_page_dissipated_heat.png"
DATA_INPUTS = [
    ...
    BERUT_AUTHOR_HEAT_IMAGE_SURFACE_PATH,
    BERUT_AUTHOR_HEAT_IMAGE_RECORD_PATH,
    BERUT_AUTHOR_HEAT_IMAGE_PATH,
...
def _build_source_evidence_intake_stub():
    ...
    berut_author_heat_surface = _load_json(BERUT_AUTHOR_HEAT_IMAGE_SURFACE_PATH)
        ...
        "source_targets": [
            {
                ...
                "evidence_fields": [
                    ...
                    {"field": "row_identifier_or_table_label", "status": "partial", "value": "Official Figure 3 PPT route captured, embedded raster candidates named, semantic asset-role review completed, jpeg_2 selected as the preferred quantitative candidate, and the author-page heat image archived; tick mapping, official parity, and numeric point/curve remain open."},
                    ...
                    {"field": "author_page_heat_image_surface", "status": "partial", "value": f"local_image={berut_author_heat_surface['source_surface']['local_image_path']}; sha256={berut_author_heat_surface['source_surface']['image_sha256']}; next={berut_author_heat_surface['blocker_transition']['new_controller']}"},
                    {
                    ...
                    },
                    {
                        ...
                        "status": "complete",
                        "value": "figure_level_locator_capture selected; official Figure 3 PPT route, embedded raster candidates, automated panel-frame candidates, semantic asset-role review, and author-page heat image surface captured; axis tick mapping, official Figure 3 parity, point/curve transcription, or stronger source-data surface still required",
                    },
                    {"field": "unit_basis", "status": "complete", "value": "T in K; heat in J with optional kT lower-bound context"},
                    {
                        ...
                        "value": (
                            ...
                            "now exposes an official Figure 3 PPT route, embedded raster candidates, a digitization protocol, candidate panel-frame coordinates, a semantic asset-role review that selects jpeg_2 for quantitative work, and an archived author-page heat image, but the source still lacks tick mapping, author-image-to-official-Figure-3 parity, and a selected numeric point/curve, so source-lock closure requires tick mapping plus transcription or one stronger upstream source-data surface before normalization."
                        ),
                    },
                ],
            },
...
def _build_evidence_lanes(test_results, metrics, readiness_matrix):
    ...
    return {
        ...
        "uet_bridge_hypothesis": {
            ...
            "blockers": [
                "Berut now has an official Figure 3 PPT route, embedded raster asset inventory, digitization protocol, automated panel-frame candidates, semantic asset-role review selecting jpeg_2, and an archived author-page heat image, but still lacks axis tick mapping, author-image-to-official-Figure-3 parity, numeric point/curve selection, and transcription or one stronger upstream source-data surface.",
        ...
        "rows": [
            {
                ...
                "uncertainty_propagation_note": "Topic-local summary row exists and Figure 3 now has an official PPT route, embedded raster inventory, digitization protocol, automated panel-frame candidates, semantic asset-role review selecting jpeg_2, and an archived author-page heat image, but tick mapping, official parity, point/curve selection, and stronger source-data provenance remain open."
            },
            {
    ...
    derivation_map = {
        ...
        "required_derivation_steps": [
            ...
            },
            {
                ...
                "needed_evidence": [
                    "Berut official Figure 3 PPT path now has an archived author-page heat image adjunct and needs axis tick mapping, author-image-to-official-Figure-3 parity, point/curve selection, and numeric transcription or one stronger upstream source-data surface",
...
def _write_verification_artifact(test_results, plot_paths, metrics):
    ...
    warnings.append(
        "Berut/CODATA source records and source-lock hashes are pinned, and the current Berut summary row now has an official Figure 3 PPT route, embedded raster inventory, digitization protocol, automated panel-frame candidates, semantic asset-role review selecting jpeg_2, and an archived author-page heat image, but tick mapping, official parity, numeric-point/curve transcription, or stronger source-data closure is still open."
    )
    warnings.append(
