...
| Jun uncertainty gap | Dedicated Jun note now states that the pinned Jun summary quantity has a first-pass interval and that the legacy `0.028 eV` row is demoted out of active Jun logic, but still lacks source-summary file/table identity for stronger source-normalized use | `JUN_2014_UNCERTAINTY_GAP.md`, `Data/03_Research/jun_2014_uncertainty_gap.json` | Keep the source-facing Jun interval while tightening the original-file/table identity and exact fit-target locator for the summary quantity. |
