...
| Jun uncertainty gap | Dedicated Jun note now states that the pinned Jun summary quantity now has a first-pass interval, but still lacks the branch/file-identity closure needed for stronger source-normalized use | `JUN_2014_UNCERTAINTY_GAP.md`, `Data/03_Research/jun_2014_uncertainty_gap.json` | Keep the source-facing Jun interval, while separating the legacy `0.028 eV` row and tightening the original-file or table identity for the summary quantity. |
