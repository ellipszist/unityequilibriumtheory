...
- `Jun` is no longer blocked by the legacy-row policy choice itself: the inherited `0.028 eV` row is now demoted to legacy context outside the active Jun lane. The remaining Jun controller is the original file/table identity and exact fit-target locator for the pinned asymptotic-work summary.
...
| Jun lower-bound summary row | pinned source summary now has a summary-layer propagated interval and the legacy 0.028 eV row is demoted out of active Jun logic | archive the source-summary file/table identity and exact fit-target locator tightly enough for stronger closure |
