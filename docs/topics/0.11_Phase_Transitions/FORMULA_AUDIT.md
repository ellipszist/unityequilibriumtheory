...
| `PT-CONSERVED-ORDER-SPECTRAL-TEX-FORMULA-FRAGMENTS` | extract exact TeX formula fragments for fixed-magnetization, canonical finite-size, and Cahn-Hilliard structure-factor policy review | `structure_factor_tex_formula_fragments.json`; `Research_Structure_Factor_Tex_Formula_Fragment_Gate.py`; `0_11_structure_factor_tex_formula_fragment_gate.json` | source TeX formulas; UET lattice units not mapped | `formula_fragment_extraction` | `fragments extracted; estimator policy blocked` | formula-source gate | Wave 43 preserves 19 fragments from 3 source lanes; current rerun warns because the temporary source cache is missing, and accepts no estimator policy. | Map fragments into UET normalization and finite-size admissibility, then accept/reject estimator replacement before exponent claims. |
| `PT-CONSERVED-ORDER-SPECTRAL-SOURCE-ARCHIVE-POLICY` | record source archive policy, expected hashes, and candidate repo paths for the three formula source archives | `structure_factor_source_archive_policy.json`; `Research_Structure_Factor_Source_Archive_Policy_Gate.py`; `0_11_structure_factor_source_archive_policy_gate.json` | provenance metadata; raw archives missing | `source_archive_policy` | `policy recorded; source availability blocked` | source-provenance gate | Wave 44 passes policy-manifest and fragment-preservation gates, but repo and temporary archive availability are both `0/3`. | Reacquire/archive raw source archives and verify hashes before fresh formula extraction or estimator-policy mapping. |
...
The source-archive policy gate records acquisition URLs, expected byte counts and hashes, and candidate repo archive paths for the three TeX source archives. It preserves the 19 extracted formula fragments as review evidence, but it does not make fresh extraction repo-reproducible because both repo archive and temporary cache availability are `0/3`. The next controller is source reacquisition or repo archival before estimator-policy mapping.

## Wave 43 Formula-Fragment Boundary

The TeX formula-fragment gate preserves 19 exact formula fragments from the three localized source lanes: fixed-magnetization effective-field/free-energy, canonical finite-size corrections, and Cahn-Hilliard/interconversion structure factor. This narrows the blocker from missing formula extraction to source-cache reproducibility, policy acceptance, and normalization mapping.
