...
| Wave 43 TeX formula-fragment extraction | Source-formula extraction gate | `Data/03_Research/structure_factor_tex_formula_fragments.json`; `Result/artifacts/0_11_structure_factor_tex_formula_fragment_gate.json` | 19 formula fragments extracted from 3 localized TeX sources; estimator policy and UET normalization remain blocked |
...
- Wave 43 extracts 19 TeX formula fragments across the fixed-magnetization, canonical finite-size, and Cahn-Hilliard structure-factor source lanes, but accepted estimator policy and UET normalization mapping remain blocked.
