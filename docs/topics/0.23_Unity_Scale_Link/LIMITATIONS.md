...
- The economy diagnostic can now explicitly disagree with the synthetic neural ordering; that mismatch is a model-hardening signal, not a nuisance to hide.
...
- `0.23` may inherit only accepted exports from `0.13`'s foundation claim gate. It cannot inherit `T13_EXPORT_UET_BRIDGE_PROOF`, source-normalized Landauer dataset closure, or Cattaneo external validation while those exports remain blocked.
...
| Cross-domain prediction | synthetic/source-referenced local pattern check with possible directional mismatch | Real external EEG/finance/cosmology source packages, reproducible retrieval, and held-out tests. |
| Information-energy inheritance | lower-bound/formula-consistency constraints from accepted `0.13` exports | `0.13` foundation gate must close source-normalized data, uncertainty, and UET derivation blockers. |
