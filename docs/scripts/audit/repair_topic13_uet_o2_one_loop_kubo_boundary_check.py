"""Make the one-loop audit check Kubo emission, not blocker wording."""
...
def main() -> int:
    ...
    if old not in text:
        raise SystemExit("one-loop Kubo boundary check not found")
    AUDIT.write_text(text.replace(old, new, 1), encoding="utf-8")
    print("PATCHED_T13_UET_O2_ONE_LOOP_KUBO_BOUNDARY_CHECK")
    return 0
